...
<div class="container-fluid">
  <div class="row bg-white">
    <div class="col-md-12 p-5">
      <div class="row p-3 SingleFollow" style="cursor: pointer" v-for="item in Follower" :key="item.userId" @click="goToUserProfile(item.userId)">
...
export default {
  ...
  methods:{
    ...
    // },
    getFollower() {
      this.$axios.post(`Follower/GetByUserId?userId=${this.$route.query.userId}`, null, ).then((response) => {
        if (response.status === 200) {
          console.log(response.data)
          response.data.forEach((item)=>{
            this.Follower.push(item.userInfo)
          })
