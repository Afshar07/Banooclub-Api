    ...
    <div class="custom_card py-3">
        ...
        <div class="d-flex">
          <div class="d-flex flex-column" @click="goToServiceProfile(post_details.userInfo)">
            <img
              ...
              :src="`https://banooclubapi.oregonserver.com/media/gallery/profile/${post_details.userInfo.selfieFileData}`"
              alt=""
            ...
            <img
              ...
              class="IMG-FLUID myPicture tw-rounded"
              :src="`https://banooclubapi.oregonserver.com/${post_details.userInfo.selfieFileData}`"
              alt=""
              style="width: 40px; height: 40px;border-radius: 50%; object-fit: contain; cursor: pointer"
            />
            <img
      ...
      >
        <vueper-slide v-for="(media,idx) in post_details.medias" :key="idx" :image="media.image">
          <template #content>
            <img class="post_images" v-if="media.priority === 2" :src="`https://banooclubapi.oregonserver.com/media/gallery/post/${media.base64}`"/>
            <video class="post_images" v-else-if="media.priority == 3" controls :src="`https://banooclubapi.oregonserver.com/media/gallery/post/${media.base64}`"></video>
          </template>
        </vueper-slide>
      </vueper-slides>
