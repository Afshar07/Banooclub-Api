<template>
  <div class="d-flex align-items-center profile_item p-2 " style="cursor: pointer!important;">
    <img v-if="UserData.userPhoto!==null && UserData.userId!==0" style="width: 50px; height: 50px;" class="rounded-circle" :src="`https://banooclubapi.simagar.com/media/gallery/profile/${UserData.userPhoto}`"/>
    <img v-else-if="UserData.userPhoto!==null && UserData.groupId!==0" style="width: 50px; height: 50px;" class="rounded-circle" :src="`https://banooclubapi.simagar.com/media/gallery/group/${UserData.userPhoto}`"/>
