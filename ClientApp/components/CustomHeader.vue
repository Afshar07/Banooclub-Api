    ...
    <div class="m-0 p-0 row col-12">
      <div class="m-0 p-0 row col-md-12 mb-md-4 d-md-block" :class="isChatLayout ? 'd-none' : 'd-block'">
        <div class="w-100 col-12 BannerPhoto d-flex flex-lg-column justify-content-end"
             style="height: 285px; background-position: center;border-top-right-radius: 1rem ;border-top-left-radius: 1rem ;" :style="InlineBg">
          <div class="row col-12 m-0 p-0">
            <div class="col-lg-12 m-0 col-12 justify-content-lg-start justify-content-end" :class="{ TopGap: this.$route.query.userId }">
              <div class="row m-0 col-12 h-100 d-flex flex-column justify-content-lg-center justify-content-end align-items-center position-relative profile_section">
                ...
                <div class="d-flex flex-column align-items-center justify-content-center">
                  <div class="d-flex flex-column align-items-center"  v-if="!$route.params.slug">
                    <h1 v-if="$auth.user && $auth.user.baseData && $auth.user.baseData.name" class="text-center mb-0" style="white-space: nowrap" >
                      {{$auth.user.baseData.name + " " + $auth.user.baseData.familyName }}
                    </h1>
                    <small class="text-primary tw-cursor-pointer" @click="goToUserProfile($auth.user.baseData)" style="text-align: center">
                  ...
                  <div class="d-flex flex-column align-items-center"  v-else-if="$route.params.slug && getUserDetails && getUserDetails.baseData">
                    <h1  class="text-center mb-0" style="white-space: nowrap">
                      {{getUserDetails.baseData.name + " " + getUserDetails.baseData.familyName }}
                    </h1>
                    <small  class="text-primary tw-cursor-pointer" @click="goToUserProfile(getUserDetails.baseData)" style="text-align: center">
                      {{getUserDetails.baseData.userName}}@
                    </small>
                    <div class="d-flex mb-4" v-if="$route.params.slug && getUserDetails" style="position: relative;top: 16px;">
                      <div class="mx-2 d-flex" style="align-items: center">
                        <div class="tw-pl-2" style="font-size: 13px">
      ...
      <ul class="nav nav-pills align-items-end profile_tabs tw-overflow-x-auto px-3" id="pills-tab" role="tablist" style="z-index: 99;">
        <li class="nav-item"  style="margin: 0 !important;">
          <button @click="$emit('SetActiveTab',1)" class="nav-link active customFontSize" id="posts-home-tab" data-bs-toggle="pill" data-bs-target="#posts-home"
            ...
            <span class="d-flex" v-else-if="$route.params.slug && getUserDetails && getUserDetails.baseData">
              ...
              <svg v-if="getUserDetails && getUserDetails.userInfo && getUserDetails.userInfo.userSetting && getUserDetails.userInfo.userSetting.isPrivatePost" xmlns="http://www.w3.org/2000/svg" class="tw-h-4 tw-w-4" viewBox="0 0 20 20" fill="#ff6f9e">
        ...
        <li class="nav-item" role="presentation m-0" style="margin: 0 !important;">
          <button @click="$emit('SetActiveTab',2)" class="nav-link  customFontSize" id="services-tab" data-bs-toggle="pill" data-bs-target="#services" type="button" role="tab" aria-controls="services" aria-selected="true">
        ...
        <li  class="nav-item" role="presentation m-0" style="margin: 0 !important;">
          <button @click="$emit('SetActiveTab',3)" class="nav-link customFontSize" id="pill-Ads" data-bs-toggle="pill" data-bs-target="#Ads"
        ...
        <li v-if="!$route.params.slug" class="nav-item" role="presentation m-0" style="margin: 0 !important;">
          <button @click="$emit('SetActiveTab',4)" class="nav-link customFontSize" id="pills-profile-tab" data-bs-toggle="pill" data-bs-target="#pills-profile"
        ...
        <li class="nav-item" role="presentation m-0" style="margin: 0 !important;">
          <button @click="$emit('SetActiveTab',5)" class="nav-link customFontSize" id="pills-contact-tab" data-bs-toggle="pill" data-bs-target="#pills-contact"
            ...
            <span class="d-flex" v-else>
              ...
              <svg v-if="getUserDetails && getUserDetails.userInfo && getUserDetails.userInfo.userSetting && getUserDetails.userInfo.userSetting.isPrivateGalleryFriend" xmlns="http://www.w3.org/2000/svg" class="tw-h-4 tw-w-4" viewBox="0 0 20 20" fill="#ff6f9e">
                <path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd" />
              </svg>
            </span>
          </button>
...
export default {
  ...
  computed: {
    InlineBg() {
      ...
      if (!this.$route.params.slug && this.$auth.user && this.$auth.user.baseData && this.$auth.user.baseData.banner) {
        ...
        return {
          backgroundImage: `url(${this.getUserDetails.baseData.banner ? base + 'media/gallery/banner/'+ this.getUserDetails.baseData.banner : "/defaultBanner.jpg"})`,
          backgroundPosition: "center",
          backgroundSize: "cover",
          backgroundRepeat: "no-repeat",
        }
    ...
    userAvatar() {
      if (!this.$route.params.slug && this.$auth.user && this.$auth.user.userInfo && this.$auth.user.userInfo.selfieFileData) {
        ...
        return this.getUserDetails.baseData.selfie ? this.BaseUrl + 'media/gallery/profile/'+this.getUserDetails.baseData.selfie : "/defaultUser.png";
      }
    },
    BaseUrl() {
  ...
  methods: {
    ...
    async SendFirstMessage(user){
      console.log(user)
      this.$router.push({path:'/Social/Chat',query:{userId:user.userInfo.userId,Photo:user.userInfo.selfieFileData}})
    },
