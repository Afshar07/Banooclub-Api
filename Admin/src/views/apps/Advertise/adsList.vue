  ...
  >
    <div v-if="Ads !== null">
      <b-modal
          ...
          @ok="ChangeAdStatus"
      >
        <span>آیا از غیر فعال کردن این آگهی اطمینان دارید ؟ </span>
      </b-modal>
        ...
        >
          ...
          <template #cell(photos[0].base64)="data">
            <img v-if="data.item.photos.length>0"
                 :src="`https://api.banooclub.simagar.com/${data.item.photos[0].base64}`" width="75px" height="75px"
                 alt=""
            >
...
export default {
  ...
  methods: {
    ...
    async getAllAds() {
      let _this = this
      let getAllAds = new GetAllAds(_this)
      let data = {
        pageNumber: _this.currentPage,
        count: _this.count,
        search: _this.search
      }
      getAllAds.setParams(data)
      await getAllAds.fetch(function (content) {
